...
<body>
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{url('browse_outgoing')}}">Browse outgoing post</a>
        </li>
        @if (Auth()->user()->admin == 'yes')
        <li class="nav-item">
    ...
    <h3 style="color:green;margin:0 0 50px 0;">Browse outgoing post</h3>
        <table id="desktop" class="table table-bordered mb-5">
            <thead>
                <tr class="bg-danger text-white">
                    ...
                    <th>Receiver's adress</th>
                </tr>
            </thead>
            @foreach($letters as $letter)
            <tr>
                <td style="vertical-align:middle">{{ $letter->id }}</td>
                <td style="vertical-align:middle">{{ $letter->send_date }}</td>
                <td style="vertical-align:middle">{{ $letter->letter_num }}</td>
                <td style="vertical-align:middle">{{ $letter->recipient }}</td>
        ...
        <table id="mobile" class="table-bordered mb-5" style="width:100%; margin:0 auto;">
                <tr>
                    <th class="bg-danger text-white" style="width:30%">#</th>
                    <td class="vertical-align:middle" style="width:70%">{{ $letter->id }}</td>
                </tr>
                <tr>
                    <th class="bg-danger text-white">Send date</th>
                    <td style="vertical-align:middle">{{ $letter->send_date }}</td>
                </tr>
                <tr>
                    <th class="bg-danger text-white">Letter number</th>
                    <td style="vertical-align:middle">{{ $letter->letter_num }}</td>
                </tr>
                <tr>
                    <th class="bg-danger text-white">Receiver's name</th>
                    <td style="vertical-align:middle">{{ $letter->recipient }}</td>
                </tr>
                <tr>
                    <th class="bg-danger text-white">Receiver's adress</th>
                    <td style="vertical-align:middle">{{ $letter->street }} {{ $letter->house_num }}<br>{{ $letter->post_code }} {{ $letter->city }}</td>
                </tr>
        </table>
        @endforeach
